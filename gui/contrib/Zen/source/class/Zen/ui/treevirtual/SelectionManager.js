qx.Class.define("Zen.ui.treevirtual.SelectionManager", {
  extend : qx.ui.table.selection.Manager,//qx.ui.treevirtual.SelectionManager,
  ...
  members : {
    ...
    _handleExtendedClick : function(tree, evt, node, left) {
      ...
      // current node was not previously selected, exiting
      if (!node.bSelected) return;
      
      // is last click timestamp not setted ?
      if (!node.lastClick) {
      ...
      // last click timestamp was setted,
      // how many ms between theses 2 clicks ?
      var diff = Date.now()-node.lastClick;

      // does the delay is reasonable for enable edit mode ?
      if (diff>300 && diff < 800) {
        // Yes, we are suppose to go to edit mode at this point
        console.log('edit mode will be enabled for node',tree,node);
